...
Диспетчер может вывести остаток времени до срабатывания самого ближайшего таймера. Если весь проект построен на периодических задачах и должен спать в промежутках между ними, то тут как раз поможет данная фича:

```cpp
void loop() {
    LP.loop();
    sleep_function(LP.nextTimerLeft());
}
```

здесь `sleep_function` - абстрактная функция, которая принимает время сна МК в миллисекундах, а `LP.nextTimerLeft()` - количество миллисекунд до ближайшего таймера. Особенности:

- Порядок работы - как в примере выше: **в глобальном цикле** вызвать `LP.loop()`, а после него считать остатки и спать
- Игнорируются:
  - Отключенные `disable()`-задачи
  - Остановленные `stop()`-таймеры `LoopTimer`
  - `LoopListener`-ы
- Учитывается время:
  - Запущенных таймеров `LoopTimer`
  - Потоков `LoopThread` в состоянии `LP_DELAY`
- Функция вернёт `0` (спать не нужно), если:
  - Ближайшее время до следующего вызова меньше либо равно 0 мс
  - Нет запущенных таймеров или ожидающих потоков
  - Есть хотя бы одна задача в очереди на добавление или удаление
  - Есть хотя бы один `LoopTicker`
...
LP_TICKER([]() {
    ...
    // таймер сработал. Остановить
    if (tmrTimeout.timeout()) {
        Serial.println("timeout");
    }

    // таймер сработал. Перезапустить
